...
class SeatsManager {
    ...
    switch (cabinNumber) {
      ...
      case 2:
        return {
          ...
          description:
            "For pregnant women, women with luggage, disabled, and elderly",
          type: "priority",
        };
      default:
        return {
  ...
  renderCabins() {
    ...
    for (let cabinNumber = 1; cabinNumber <= 5; cabinNumber++) {
      const cabinSeats = this.seats.filter(
        (seat) => seat.cabin === cabinNumber,
      );
  ...
  updateUserInfo() {
    ...
    const typeText =
      this.user.userType === "general"
        ? "General"
        : this.user.userType.charAt(0).toUpperCase() +
          this.user.userType.slice(1);

    userTypeBadge.textContent = typeText;
    passengerType.textContent =
      this.user.userType === "general"
        ? "General Passenger"
    ...
    if (this.user.userType === "women" || this.user.userType === "pregnant") {
      if (this.user.hasLuggage) {
        accessText =
          "You have access to women-only cabin (Cabin 1) and priority cabin (Cabin 2)";
      } else {
