...
class AdminDashboard {
    ...
    bindEvents() {
        ...
        // Export button
        document.getElementById('exportPassengersBtn').addEventListener('click', () => {
    ...
    async loadAllData() {
        try {
            ...
            const [statsResponse, passengersResponse, bookingsResponse] = await Promise.all([
    ...
    updateStatsDisplay() {
        document.getElementById('totalPassengers').textContent = this.stats.unique_passengers;
        document.getElementById('totalBookings').textContent = this.stats.total_bookings;
        document.getElementById('totalRevenue').textContent = `$${this.stats.total_revenue.toFixed(2)}`;
        document.getElementById('bookingsToday').textContent = this.stats.bookings_today;
    }
    ...
    updateCabinOccupancy(cabinData) {
        ...
        cabinStats.innerHTML = cabinData.map(cabin => {
            const occupancyRate = parseFloat(cabin.occupancy_rate) || 0;
            const fillColor = occupancyRate > 80 ? '#ef4444' : 
                             occupancyRate > 60 ? '#f59e0b' : 
                             '#10b981';
    ...
    updatePassengersTable() {
        ...
        if (this.passengers.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="6" class="loading-row">No passengers found</td></tr>';
            return;
        }

        tableBody.innerHTML = this.passengers.map(passenger => `
            <tr>
    ...
    updateRecentBookings() {
        ...
        if (this.recentBookings.length === 0) {
            recentBookings.innerHTML = '<div class="loading-message">No recent bookings</div>';
            return;
        }

        recentBookings.innerHTML = this.recentBookings.slice(0, 10).map(booking => `
    ...
    exportPassengerData() {
        ...
        // Convert to CSV
        const headers = ['Email', 'User Type', 'Has Luggage', 'Total Bookings', 'Total Spent', 'Joined Date'];
        const csvContent = [
            headers.join(','),
