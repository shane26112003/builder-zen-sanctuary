...
export const handleBookSeats: RequestHandler = async (req, res) => {
  try {
    ...
    if (!userId || !seatIds || !Array.isArray(seatIds) || seatIds.length === 0) {
      return res.status(400).json({ error: 'User ID and seat IDs are required' });
    }

    // Verify user exists
    const user = await db.pool.query('SELECT * FROM metro_users WHERE id = $1', [userId]);
    ...
    for (const seat of seatDetails.rows) {
      ...
      if (seat.cabin === 1 && userData.user_type !== 'women' && userData.user_type !== 'pregnant') {
